...
def init(address, authkey):
    ...
    logger.info("U need to Start video server")
    ...
    #           f"--port {address[1]} "\
    #           f"--authkey {authkey}"
    #subprocess.Popen('python3', shell=True)
...
input_path = '/mnt/hdd/huy_workspace/Base_solution/AK_base_linux_port/interface/video/python/video_rgb565.raw'
...
class Huy_Video:
    ...
    def readFrame(self):
        data = self.f.read(FRAME_SIZE)
        eos = len(data) < FRAME_SIZE
        return data, eos
...
def rdDataDMA(size):
    ...
    if (STATUS & STATUS_ACTIVE_Msk) != 0:
        ...
        huy_video = Huy_Video(input_path)
        #data, eos = Video.readFrame() #######

        ## This does not work
        # while True:
        #     data, eos = huy_video.readFrame()
        #     if eos:
...
def wrCONTROL(value):
    ...
    if ((value ^ CONTROL) & CONTROL_ENABLE_Msk) != 0:
        ...
        if (value & CONTROL_ENABLE_Msk) != 0:
            logger.info("Configure video stream at w:{} h: {} mode: {}".format(FRAME_WIDTH, FRAME_HEIGHT, MODE))
...
def wrRegs(index, value):
    ...
    if   index == 0:
        ...
        logger.info("Maybe start video stream")
        wrCONTROL(value)
    elif index == 2:
        value = STATUS
    elif index == 3:
